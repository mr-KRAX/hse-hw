...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    f_sorted = feature_vector[indices_sorted]
    t_sorted = target_vector[indices_sorted]

    dups = f_sorted[1:] != f_sorted[:-1]
    thresholds = ((f_sorted[1:] + f_sorted[:-1]) / 2)[dups]

    R = np.size(t_sorted)
    R_l_size = np.arange(1, R)
    R_l_1 = np.cumsum(t_sorted)
    R_l_p1 = R_l_1[:-1] / R_l_size
    R_l_p0 = 1 - R_l_p1

    R_r_size = R - R_l_size
    R_r_1 = R_l_1[-1] - R_l_1[:-1]
    R_r_p1 = R_r_1 / R_r_size
    R_r_p0 = 1 - R_r_p1

    ginis = -R_l_size / R * (1 - R_l_p0**2 - R_l_p1**2) - \
        R_r_size / R * (1 - R_r_p0**2 - R_r_p1**2)
    ginis = ginis[dups]
        ...
        for feature in range(sub_X.shape[1]):
            ...
            if feature_type == 'real':
                ...
                sorted_categories = sorted(
                    ratio.keys(), key=lambda k: ratio[k])
